...
- **Physical Android device times out connecting to Windows:** confirm both devices use the same non-guest Wi-Fi/LAN and that client isolation is disabled. In an elevated PowerShell window, allow Orbit's QUIC port with `New-NetFirewallRule -DisplayName "Orbit QUIC" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 48177 -Profile Private`. Keep the Windows listener at `0.0.0.0:48177`, but advertise the PC's current LAN address in invitations; if DHCP changes it, create and import a new invitation to update the saved peer address.
